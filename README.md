...
- `src/data/*.json`과 `public/guide-images`는 `deno task crawl:guide`로 생성하며 추적하지 않습니다. CI는 생성된 데이터를 `guide-data` artifact로 보관하고, 나무위키 차단 시 해당 cache를 먼저 사용한 뒤 없으면 `deno task restore:guide` seed를 사용합니다.
